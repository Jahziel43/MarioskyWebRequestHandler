...
regresar solo la página de inicio. El html de esta página esta en el archivo `home.html`.
El sitio es el **home page** de una estudiante de la materia de Programación Web. Modifica el código
del archivo para que el servidor regrese el contenido del archivo `home.html` en la ruta `/`.
...
de estilo CSS. Por lo tanto, no te preocupes si la página se ve un poco austera (no es malo y depende de gustos).

En caso de que el usuario solicite una ruta distinta el servidor debe regresar un **error 404**. Investiga, si todavía
...
También puedes abrir un navegador y poner la dirección `http://localhost:8000` en la barra de direcciones.
...
Una manera de implementar este sitio sencillo es almacenar el html correspondiente en un diccionario:
...
Debes utilizar diccionarios de Python para implementar esta funcionalidad.
...
## Cambiando desde servidor remotamente